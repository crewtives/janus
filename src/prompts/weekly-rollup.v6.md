...
days: <%= it.days %>
tags: [weekly-rollup, rollup/<%= it.endDate.slice(0, 7) %>, type/weekly]
...
**No date-chain to the daily rollup and no MOC footer** — those bridged the Timeline into every project. The dominant-track `[[<project>]]` hub links in §2 are the legitimate cross-project references; keep only the dashboard entry point here:
