...
## Coding-agent sessions (<%= it.sessions.length %>)

<% if (it.sessions.length === 0) { %>
(no sessions recorded today)
<% } else { %>
<% it.sessions.forEach(function(s) { %>
### <%= s.firstTimestamp || "?" %> — <%= s.source %> session `<%= (s.sessionId || "????????").slice(0,8) %>`
...
- `track/<slug>` tags can be added to the frontmatter `tags:` field (e.g. `tags: [pulse, pulse/<%= it.project %>, type/pulse, project/<%= it.project %>, track/globex-checkout-moderno]`).
<% } %>
...
tags: [pulse, pulse/<%= it.project %>, type/pulse, project/<%= it.project %>]   # optional: add `track/<slug>` per tagged track
...
The pulse keeps **exactly one** structural up-link — to its project hub. No MOC footer, no previous/next date-chain: chronology lives in the `prev:`/`next:` frontmatter that the system stamps deterministically, and MOC links live on the hub/index, not on pulses.
...
7. Wiki-links only for `[[<%= it.project %>]]` (the one structural up-link) and the previous pulses I EXPLICITLY pass for prose cross-refs (in `previousRisks`, `previousDecisions`, and the "this day, last year" anchor). DO NOT invent pulse filenames. Don't add a MOC footer or a previous/next date-chain link — those are handled elsewhere.
