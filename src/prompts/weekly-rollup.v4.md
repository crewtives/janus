...
El sistema detectó loops abiertos que llevan días sin mover. Incluí este callout EN EL WEEKLY como sección dedicada (después del TL;DR, antes de "Tracks dominantes"), **literal**:
...
Reglas:
- No parafrasees el contenido del callout (los días/fechas son reales y vienen de la DB).
- Si en el TL;DR de la semana surge un track dormido del listado, mencionalo de forma honesta (no "todo se cerró bien" si hay loops abiertos).
- No agregues loops que no estén en el callout — esta es la fuente única.
<% } %>

<% if (it.patternsCallout) { %>
# PATTERNS AUTO-DETECTADOS

Un pass pre-weekly detectó los siguientes patrones implícitos. Inyectalos como callout dedicado (después de "Tracks dominantes", antes de "Top outcomes"):
...
Reglas:
- El callout va con el contenido **literal** — el sistema ya filtró por confidence > 0.6.
- Si en el TL;DR de la semana mencionás algún patrón, hacelo con honestidad. Los patterns son señal, no narrativa central.
<% } %>

<% if (it.stuckBlockers && it.stuckBlockers.length > 0) { %>
# STUCK PATTERNS

Los siguientes blockers vienen apareciendo en weeklies consecutivos sin moverse. Incluí un callout dedicado al final (antes de Navegación):

```
> [!danger] Stuck patterns
<% it.stuckBlockers.forEach(function(b) { %>> - "<%= b.text %>" — apareció en <%= b.weeklyCount %> weeklies consecutivos (desde <%= b.firstSeen %>)
<% }) %>
```

Pasaron el umbral del weekly N + 1 sin resolverse. El callout no celebra: nombra el patrón y deja explícito que el sistema escaló.
<% } %>
...
### 8. Preguntas para vos (reflection prompts)

Sección final reflexiva antes de la navegación. Bajá la guardia de "narrador objetivo" y formulá 2-3 preguntas honestas que ayuden al usuario a pensar la semana. Variá las preguntas semana a semana — elegí del set siguiente las que aplican según lo que pasó:

- "¿Qué te sorprendió esta semana?" — siempre relevante.
- "¿Qué tema cruzó proyectos sin que lo planearas?" — cuando aparecen tracks cross-proyecto en el lineage.
- "¿Qué se trabó dos veces?" — cuando hay riesgos persistentes o stuck patterns.
- "¿Qué pulse de esta semana te gustaría re-leer dentro de 3 meses?" — cuando hubo decisions ADR-candidate o cambios estructurales.
- "¿Hay algún open loop que ya no querés cerrar?" — cuando hay tracks dormidos > 14 días.

Forma:

```
> [!question]+ Preguntas para vos
> 1. <pregunta 1 — adaptada al contexto de la semana>
>
> 2. <pregunta 2 — distinto ángulo>
>
...
**CRÍTICO — preservación de respuestas del usuario**:
- Si en la versión actual del weekly el usuario ya respondió debajo de una pregunta, **preservá ese texto literal**. La regeneración NO sobreescribe respuestas.
- Si no hay respuesta previa, dejá el placeholder `_(espacio para responder — preservado en regeneraciones)_`.
- Mantené el `[!question]+` (expandible por default) para que la sección sea visible.
