...
- R2. A card's provenance comes from the mirror's own frontmatter, which already records it: a mirror that is not marked as needing review is *reconciled*, and its cards render as such; a mirror sourced from pulse inference renders its cards as *inferred*, visibly marked; a mirror that is still a pending placeholder is *no source*.
...
- R4. A project whose mirror is missing, is a pending placeholder, or yields zero checkbox lines renders a visible placeholder naming what would populate it, rather than rendering nothing.
- R5. Projects with `status: "archived"` contribute no cards. Every other status, including `paused`, contributes normally.

**Board integrity**

- R6. Every input has an explicit outcome. Per project: reconciled mirror, inferred mirror, no source, or unreadable. For the blocked lane: rows in window; *no weekly recorded in window*, which renders as unknown rather than as an empty lane; or *unreachable*, when the state database is absent — which is distinct from a query returning zero rows.
...
- KTD13. **No agent-facing surface in v1.** `AGENTS.md` admits new MCP tools for data Janus already indexes, and KTD7 keeps the board out of the index — so a board tool is blocked on reversing KTD7, not merely deferred. When a concrete consumer appears, the cheaper shape is a field inside the existing project-context tool, which the server's instructions already steer agents to call before substantive work: it reaches every session, touches no pinned tool count, and needs no indexing. The pinned count is a cost, not the reason.
