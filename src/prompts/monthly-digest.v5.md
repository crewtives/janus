...
days: <%= it.days %>
tags: [monthly, monthly/<%= it.month %>, type/monthly]
...
> 1. **<area>** [<project>] — <concrete outcome + why it matters, in one dense line> · `<sha7>` (or a plain `YYYY-MM-DD` date — NOT a `[[pulse]]` wiki-link)
> 2. ...
```
...
- **Reference weeklies**: <YYYY-MM-DD>, <YYYY-MM-DD> (plain dates — NOT `[[…-week]]` wiki-links)
```

Same track format as weekly v6 — the `materializeTracks` script will also process them. **Projects** keeps its `[[<project>]]` hub links (the legitimate cross-project reference); everything else in the monthly is plain text.
...
> - **<short decision>** [<project>] — <context/reason in one line> · <YYYY-MM-DD> (plain date, NOT a wiki-link)
> - ...
```

Max 5. If a decision reverts/modifies an earlier one from the same month, mark it: "**reverts** decision from <YYYY-MM-DD>" (plain date).
...
**No month-to-month date-chain, no per-weekly wiki-links, no MOC footer** — those bridged the Timeline into every project. Keep only the dashboard entry point:
