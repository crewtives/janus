...
async function setupVault(dates: PulseSpec[]): Promise<{ obsidianPath: string; cleanup: () => Promise<void> }> {
...
describe("compactIdleStreaks", () => {
  ...
  test("dryRun does not touch files", async () => {
    ...
    const r = await compactIdleStreaks({ obsidianPath, project: "test-proj", dryRun: true });
    expect(r.streaksFound).toBe(1);
    expect(r.streaksWritten).toBe(0);
  ...
  test("includeBoring=true compacts boring days into a quiet-streak", async () => {
    ...
    const r = await compactIdleStreaks({ obsidianPath, project: "test-proj", includeBoring: true });
    expect(r.streaksFound).toBe(1);
