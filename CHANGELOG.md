...
- **Homebrew formula template** at `docs/distribution/homebrew/janus.rb` plus a `homebrew-bump` job in `release.yml` (guarded by `if: false`). When activated, every release auto-PRs the formula in `crewtives/homebrew-janus` so `brew install crewtives/janus/janus` keeps working.
